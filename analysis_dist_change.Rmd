...
baseline_rt_plot <- baseline_rt |>
  ...
  geom_signif(comparisons = list(c("Before Change", "After Change")),
              annotations = "***", y_position = 1200, tip_length = 0.01) +
...
interference_plot <- plot_data |>
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", 
               position = position_dodge(0.8), width = 0.2) +
  geom_signif(data = subset(plot_data, Session == "Before Change"),
              aes(xmin = 0.75, xmax = 1.0, annotations = "*", y_position = 285),
              manual = TRUE, tip_length = 0.01, textsize = 4) +
  geom_signif(data = subset(plot_data, Session == "After Change"),
              aes(xmin = 1.75, xmax = 2.0, annotations = "n.s.", y_position = 280),
              manual = TRUE, tip_length = 0.01, textsize = 3) +
  geom_signif(data = subset(plot_data, Session == "Before Change"),
              aes(xmin = 1.0, xmax = 1.25, annotations = "*", y_position = 240),
              manual = TRUE, tip_length = 0.01, textsize = 4) +
  geom_signif(data = subset(plot_data, Session == "After Change"),
              aes(xmin = 2.0, xmax = 2.25, annotations = "*", y_position = 260),
              manual = TRUE, tip_length = 0.01, textsize = 4) +
  scale_fill_manual(values = colors) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(
...
## 3.5 Questionaire Data Summary
```{r questionaire}
# load data
qdata <- read.csv("data/qdata.csv")
qdata <- mutate(qdata, Q1_aware=ifelse(Q1_resp==2, 1, 0), Q3_aware=ifelse(Q3_resp==2, 1, 0))
qdata %>% group_by(distr_cond) %>% summarize(sum(Q1_aware), sum(Q2_corr), sum(Q3_aware))
qdata %>% group_by(distr_cond, Q1_aware) %>% summarize(q2c=sum(Q2_corr), N=n(), q2c/N)

qdata_awareness <- mutate(filter(qdata, distr_cond == "unequal"), awareness = ifelse(Q1_aware == 1 & Q2_corr == 1, "Aware", "Unaware"))
int_data_awareness <- full_join(filter(interference_detailed, distr_cond == "unequal"), qdata_awareness, by = c("participant", "distr_cond"))

group_by(int_data_awareness, awareness) %>% summarize(int = mean(avg_interference), sd = sd(avg_interference), n = n(), sem = sd/sqrt(n))

# t-test for awareness effect on average interference
t.test(filter(int_data_awareness, awareness == "Aware")$avg_interference, filter(int_data_awareness, awareness == "Unaware")$avg_interference, var.equal = TRUE)

#filter(int_data_awareness, type != "avg_int") %>% ezANOVA(dv = int, wid = participant, within = type, between = awareness)
```
...
int_phases_data <- sint_data_both %>% 
  ...
  int_phases_fig = int_phases_data |>
  ...
  geom_signif(data = subset(int_phases_data, rt_type == "Response Phase"),
              aes(xmin = 1, xmax = 2.0, annotations = "***", y_position = 80),
              manual = TRUE, tip_length = 0.01, textsize = 4) +
  geom_signif(data = subset(int_phases_data, rt_type == "First Target Fixation"),
              aes(xmin = 1, xmax = 2.0, annotations = "n.s.", y_position = 250),
              manual = TRUE, tip_length = 0.01, textsize = 4) +
  scale_y_continuous(limits = c(0, NA)) +
  theme_classic() +
  theme(
